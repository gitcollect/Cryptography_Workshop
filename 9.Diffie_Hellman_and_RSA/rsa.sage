...
def assignment4():
	...
	# Da wir wissen, dass m ein ascii-Zeichen ist, raten wir jetzt Buchstaben, 
	# bis wir es haben :-)
	# So ziemlich alle Werte der Ascii-Tabelle zwischen 33 und 255 sind "printable".
	for ascii in range(33, 256):
		c = chr(ascii)
		test = pow(ord(c), e, n)
		for i in range(len(ciphertext)):
			if test == ciphertext[i]:
				plaintext[i] = c
		
	plaintext = reduceArray(plaintext)
